...
name_term:   /* returns a bool * (SmtAtom.Form.pform or a SmtAtom.hatom) (given a state), the boolean indicates if we should declare the term or not */
  ...
  | var_atvar						   { fun st qst -> let x = $1 in match find_opt_qvar x qst with
							   ...
							   | None -> true, Form.Atom (Atom.get (VeritSyntax.get_atom_tbl_to_add st) (Aapp (LocalState.get_fun (VeritSyntax.get_smt_state st) $1, [||]))) }
...
term:   /* returns a bool * (SmtAtom.Form.pform or SmtAtom.hatom), the boolean indicates if we should declare the term or not */
  ...
  | VAR                                                    { fun st qst -> let x = $1 in match find_opt_qvar x qst with | Some bt -> false, Form.Atom (Atom.get ~declare:false (VeritSyntax.get_atom_tbl_to_add st) (Aapp (dummy_indexed_op (Rel_name x) [||] bt, [||]))) | None -> true, Form.Atom (Atom.get (VeritSyntax.get_atom_tbl_to_add st) (Aapp (LocalState.get_fun (VeritSyntax.get_smt_state st) x, [||]))) }
  | VAR args                                               { fun st qst -> let f = $1 in let a = $2 st qst in match find_opt_qvar f qst with | Some bt -> let op = dummy_indexed_op (Rel_name f) [||] bt in false, Form.Atom (Atom.get ~declare:false (VeritSyntax.get_atom_tbl_to_add st) (Aapp (op, Array.of_list (snd (list_dec a))))) | None -> let dl, l = list_dec a in dl, Form.Atom (Atom.get (VeritSyntax.get_atom_tbl_to_add st) ~declare:dl (Aapp (LocalState.get_fun (VeritSyntax.get_smt_state st) f, Array.of_list l))) }
