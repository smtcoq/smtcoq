...
From SMTCoq.utils Require Import Misc PArray State Classes.
From SMTCoq.structures Require Import BVList FArray.
From SMTCoq.core Require Import Terms.
      ...
      match get_form (Lit.blit lres) with
        | FbbT a bs =>
          match get_atom a with
            | Acop (CO_BV bv N) =>
              if check_bbc bv bs && (N.of_nat (List.length bv) =? N)%N
              then lres::nil
              else C._true
            | _ => C._true
          end
...
Fixpoint check_symopp (bs1 bs2 bsres : list _lit) (bvop: binop)  :=
  ...
  Lemma empty_list_length: forall {A: Type} (a: list A), (List.length a = 0)%nat <-> a = [].
  Proof. intros A a.
  ...
  Definition check_mult (bs1 bs2 bsres: list _lit) : bool :=
   if Nat.eqb (List.length bs1) (List.length bs2)%nat then
    let bvm12 := bblast_bvmult bs1 bs2 (List.length bs1) in
...
(* bitwise disequality *)
             ...
             match (get_atom a), (get_atom b) with
               | (Acop (CO_BV bv1 n1)), (Acop (CO_BV bv2 n2)) =>
                  if List_diseqb bv1 bv2 
                     && (N.of_nat (List.length bv1) =? n)%N
                     && (N.of_nat (List.length bv2) =? n)%N
                     && (n1 =? n)%N && (n2 =? n)%N
                  then lres::nil
                  else C._true
               | _, _ => C._true
  ...
  Fixpoint extract_lit (x: list _lit) (i j: nat) : list _lit :=
   ...
   Definition check_extract (bs bsres: list _lit) (i j: N) : bool :=
     if (N.ltb (N.of_nat (List.length bs)) j)
     then false
     else
...
Lemma prop_checkbb': forall (a: int) (bs: list _lit),
                     (check_bb a bs 0 (List.length bs) = true) ->
                     (forall i0, (i0 < (List.length bs) )%nat ->
                     (Lit.interp rho  (nth i0 bs 1)) = 
                     (@BITVECTOR_LIST.bitOf (N.of_nat(List.length bs)) i0
                     (interp_form_hatom_bv a (N.of_nat (List.length bs))))).
Proof. intros.
       specialize (@prop_checkbb a bs 0 (List.length bs)).
       intros Hp.
       cut ((i0 - 0)%nat = i0%nat).
       intro Hc.
...
Lemma nth_eq: forall (a b: list bool), (List.length a) = (List.length b) ->
 (forall (i: nat), 
 (i < List.length a)%nat ->
...
Lemma bitOf_of_bits: forall l (a: BITVECTOR_LIST.bitvector (N.of_nat (List.length l))),
                               (forall i, 
                               (i < (List.length l))%nat ->
                               nth i l false = 
                               (@BITVECTOR_LIST.bitOf (N.of_nat (List.length l)) i a))
                               ->
...
Lemma check_bbc_length : forall bv bs, check_bbc bv bs = true -> List.length bv = List.length bs.
Proof.
...
Lemma prop_check_not:
  forall bs br, List.length bs = List.length br ->
...
Lemma check_not_length:
  forall bs br, check_not bs br = true -> List.length bs = List.length br.
Proof.
...
Lemma check_symopp_bvand: forall bs1 bs2 bsres N, 
  let n := List.length bsres in
  (List.length bs1 = n)%nat ->
  (List.length bs2 = n)%nat ->
...
Lemma check_symopp_bvand_length: forall bs1 bs2 bsres N,
  let n := List.length bsres in
  check_symopp bs1 bs2 bsres (BO_BVand N) = true ->
  (List.length bs1 = n)%nat /\ (List.length bs2 = n)%nat .
Proof.
  intros.
  revert bs1 bs2 N H.
       ...
       - intros.
         ...
         case bs2 in *.
           ...
           case bs2 in *.
             ...
             simpl in H0.
             assert (List.length bs1 = (n-1)%nat).
             { lia. }
...
Lemma length_check_eq: forall bs1 bs2 bsres,
  check_eq bs1 bs2 bsres = true -> List.length bs1 = List.length bs2.
Proof.
  intro bs1.
  induction bs1.
...
interp_carry (ult_big_endian_lit_list (List.rev bs1) (List.rev bs2)) = Lit.interp rho bsres.
Proof. intro bs1.
       induction bs1 as [ | xbs1 xsbs1 IHbs1].
       - intros.
         case bs2 in *.
...
interp_carry (slt_big_endian_lit_list (List.rev bs1) (List.rev bs2)) = Lit.interp rho bsres.
Proof. intro bs1.
       induction bs1 as [ | xbs1 xsbs1 IHbs1].
       - intros.
         case bs2 in *.
...
Lemma check_add_bvadd: forall bs1 bs2 bsres n, 
  (N.of_nat(List.length bs1) = n)%N ->
  (N.of_nat(List.length bs2) = n)%N ->
  (N.of_nat(List.length bsres) = n)%N ->
...
Lemma check_neg_length: forall bs bsres,
  check_neg bs bsres = true -> (List.length bs = List.length bsres)%nat.
Proof. intros.
...
Lemma prop_forallb2: forall {A B} {f: A -> B -> bool} l1 l2, forallb2 f l1 l2 = true -> List.length l1 = List.length l2.
...
List.length (mult_step_k_h a b c k) = List.length a .
Proof. intro a.
       induction a as [ | xa xsa IHa ].
       - intros. simpl. easy.
       - intros.
...
Lemma prop_mult_step3: forall k' a b res k, 
                         List.length (mult_step a b res k k') = (List.length res)%nat.
Proof. intro k'.
       induction k'.
...
Lemma prop_and_with_bit2: forall bs1 b, List.length (and_with_bit bs1 b) = List.length bs1.
Proof. intros bs1.
       induction bs1.
       - intros. now simpl.
...
Lemma check_bvmult_length: forall bs1 bs2,
  let bsres0 := bblast_bvmult bs1 bs2 (List.length bs1) in
  List.length bs1 = List.length bs2 -> List.length bs1 = List.length bsres0.
Proof. intros. unfold bblast_bvmult in bsres0.
       case_eq (List.length bs1). intros. unfold bsres0.
       rewrite H0.
...
Lemma check_bvmult_length2: forall bs1 bs2 bsres,
  check_mult bs1 bs2 bsres = true -> List.length bs1 = List.length bs2 .
Proof. intros bs1.
       induction bs1.
       - intros. case bs2 in *.
         + easy.
...
Lemma concat_len: forall (bs1 bs2 bsres: list bool), bs1 ++ bs2 = bsres ->
                              ((List.length bs1) + (List.length bs2))%nat = (List.length bsres)%nat.
Proof.
  intro bs1.
  induction bs1.
...
Lemma extract_interp_main: forall bs1 bsres (i n0: N),
                  ...
                  @RAWBITVECTOR_LIST.bv_extr i n0 (N.of_nat (List.length bs1))
...
Proof.
    ...
    - case_eq ((a1 =? a1')); simpl; intros Heq7; try (now apply C.interp_true).
        ...
        assert (H100: (N.of_nat (Datatypes.length (map (Lit.interp rho) bsres))) = n0%N).
        {
           ...
           assert (List.length (RAWBITVECTOR_LIST.extract (map (Lit.interp rho) bs1) (N.to_nat i)
           (N.to_nat (n0 + i))) = List.length (map (Lit.interp rho) bsres)).
...
Proof.
    ...
    - case_eq ((a1 =? a1')); simpl; intros Heq7; try (now apply C.interp_true).
        ...
        assert (H100: (N.of_nat (Datatypes.length (map (Lit.interp rho) bsres))) = (i + n)%N).
        {
           ...
           assert (List.length (RAWBITVECTOR_LIST.zextend (map (Lit.interp rho) bs1) (N.to_nat i))
           = List.length (map (Lit.interp rho) bsres)).
           { now rewrite Heq8a. }
...
Proof.
    ...
    - case_eq ((a1 =? a1')); simpl; intros Heq7; try (now apply C.interp_true).
        ...
        assert (H100: (N.of_nat (Datatypes.length (map (Lit.interp rho) bsres))) = (i + n)%N).
        {
           ...
           assert (List.length (RAWBITVECTOR_LIST.sextend (map (Lit.interp rho) bs1) (N.to_nat i))
           = List.length (map (Lit.interp rho) bsres)).
           { now rewrite Heq8a. }
...
Lemma helper: forall {A} (l0: list A) i0, S (S (List.length match l0 with
                       | [] => []
                       | _ :: _ => i0 :: removelast l0
                       end)) = S (S (List.length l0)).
Proof. intros A l0.
       induction l0; intros.
       - now simpl.
       - simpl. now rewrite IHl0.
...
Lemma length_shl_lit_be: forall a, List.length (_shl_lit_be a) = List.length a.
Proof. intro a.
       induction a; intros.
       - now simpl.
...
Lemma length_nshl_lit_be: forall n a, List.length (nshl_lit_be a n) = List.length a.
Proof. intros n.
       induction n; intros.
       - now simpl.
...
Lemma length_shl_be: forall a b, List.length a = List.length (shl_lit_be a b).
Proof. intros. unfold shl_lit_be. now rewrite length_nshl_lit_be. Qed.

Lemma length_check_shsl: forall bs1 bs2 bsres , 
  check_shl bs1 bs2 bsres = true -> List.length bs1 = List.length bs2 -> List.length bs1 = List.length bsres.
Proof. intro bs1.
       ...
       - simpl in *.
         ...
         +  apply prop_eq_carry_lit2 in H1.
            rewrite prop_interp_carry3 in H1.
            simpl in H1.
            assert (List.length (map (Lit.interp rho) (shl_lit_be (a :: bs1) (b :: l))) =
                    List.length (map (Lit.interp rho) bsres)).
            { now rewrite H1. }
...
Proof. intro bs1.
       ...
       - intros. simpl.
         ...
         case_eq (Nat.eqb (@List.length int ([])) (List.length bs2)); intros.
...
Lemma length_shr_lit_be: forall a, List.length (_shr_lit_be a) = List.length a.
Proof. intro a.
       induction a; intros.
       - now simpl.
...
Lemma length_nshr_lit_be: forall n a, List.length (nshr_lit_be a n) = List.length a.
Proof. intros n.
       induction n; intros.
       - now simpl.
...
Lemma length_shr_be: forall a b, List.length a = List.length (shr_lit_be a b).
Proof. intros. unfold shr_lit_be. now rewrite length_nshr_lit_be. Qed.

Lemma length_check_shsr: forall bs1 bs2 bsres , 
  check_shr bs1 bs2 bsres = true -> List.length bs1 = List.length bs2 -> List.length bs1 = List.length bsres.
Proof. intro bs1.
       induction bs1; intros.
       - case_eq bs2; simpl; intros.
         + subst. unfold check_shl in H. simpl in H.
       ...
       - simpl in *.
         ...
         +  apply prop_eq_carry_lit2 in H1.
            rewrite prop_interp_carry3 in H1.
            simpl in H1.
            assert (List.length (map (Lit.interp rho) (shr_lit_be (a :: bs1) (b :: l))) =
                    List.length (map (Lit.interp rho) bsres)).
            { now rewrite H1. }
