...
(*****)
     ...
     (** Z.eq_dec *)
      ...
      (* BO_eq *)
      case_eq (Typ.cast ti t0);
      case_eq (Typ.cast ti0 t0);
      case_eq (Typ.cast ti1 t0);
      case_eq (Typ.cast ti2 t0);
      intros;
      try (apply Typ.cast_eq in H8);
      try (apply Typ.cast_eq in H10);
      try (apply Typ.cast_eq in H11);
      try (apply Typ.cast_eq in H12);
      try (subst ti);
      try (subst ti0);
      try (subst ti1);
      try (subst ti2);
      try (rewrite Typ.cast_refl in HHa);
      try (rewrite Typ.cast_refl in HHb);
      try (rewrite Typ.cast_refl in H6);
      try (rewrite Typ.cast_refl in H7);
      try (rewrite H8 in H6; now contradict H6);
      try (rewrite H10 in H7; now contradict H7);
      try (apply Typ.nocast_refl in H11;
           rewrite H11 in H6; now contradict H6);
      try (apply Typ.nocast_refl in H12;
           rewrite H12 in H7; now contradict H7).
      ...
      case_eq (Typ.i_eqb t_i t0 vi vi0); intro.
      rewrite Typ.i_eqb_sym in H7.
      rewrite ((Typ.i_eqb_trans _ _ _ _ _ (Typ.i_eqb_trans _ _ _ _ _ H7 H10) H6)).
      easy.
      ...
      unfold not; intro.
      rewrite Typ.i_eqb_sym in H6.
      rewrite ((Typ.i_eqb_trans _ _ _ _ _ (Typ.i_eqb_trans _ _ _ _ _ H7 H11) H6)) in H10.
      now contradict H10.
      ...
      rewrite H11, H12 in HHa.
      rewrite H8, H10 in HHb.
      destruct t; try inversion HHa.
      apply Atom.Bval_inj2 in HHa.
      apply Atom.Bval_inj2 in HHb.
      rewrite <- HHa, <- HHb.
...
(*  bv_or  *) 
    ...
    destruct (t_func.[i0]) as ((t_args, t_ret), f).
    simpl in HHa, HHb.
    clear H1 H5 H4 H2 H3 H.
    revert HHa HHb.
    revert dependent f.
    revert dependent t_args.
    (* revert dependent a. *)
    (* revert dependent b. *)
    ...
    - intros.
      rewrite HHa in HHb.
      apply Atom.Bval_inj2 in HHb.
      rewrite HHb; now apply Typ.i_eqb_refl.
    ...
    - intros.
      ...
      apply Atom.Bval_inj2 in HHa.
      apply Atom.Bval_inj2 in HHb.
      subst va vb. apply Typ.i_eqb_refl.
      simpl in HHa, HHb.

      unfold i_eqb_atom in H.
      unfold Atom.interp_hatom in H.
      destruct (Atom.t_interp t_i t_func t_atom .[ a0]) as (t1, v1).
      destruct (Atom.t_interp t_i t_func t_atom .[ b0]) as (t2, v2).

      case_eq (Typ.cast t1 t0); intros.
      case_eq (Typ.cast t2 t0); intros.
      apply Typ.cast_eq in H1.
      apply Typ.cast_eq in H2.
      subst t1 t2.
      rewrite Typ.cast_refl in *.
      unfold Typ.interp_ftype in f. simpl in f.
      clear k k0.
