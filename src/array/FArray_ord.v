...
Section Array.
  ...
  Definition ltke (a b : (key * elt)) :=
    lt (fst a) (fst b) \/ ( ~ lt (fst a) (fst b) /\ lt (snd a) (snd b)).
  
  Lemma ke_ord: OrdType (key * elt).
  Proof.
    exists ltke.
    unfold ltke. intros.
    destruct H, H0.
    left; apply (lt_trans (fst x) (fst y) (fst z)); auto.
    apply (lt_trans (snd x) (snd y) (snd z)); auto.
    unfold ltke. intros. destruct H.
    ...
    apply LT. split; assumption.
    apply LT.
    apply EQ. destruct x, y. simpl in *. rewrite e, e0; trivial.


    Check (fun x y => compare (fst x) (fst y)).

    
    elim H.  destruct H. . rewrite H.
  ...
  Proof. apply InfA_ltA. exact (InfA_ltA lt_strorder). Qed.
