...
Module Raw.
  ...
  #[local]
  Hint Unfold eqk eqke : smtcoq_farray.
  #[local]
  Hint Extern 2 (eqke ?a ?b) => split : smtcoq_farray.
  ...
  Proof. auto with smtcoq_farray. Qed.
  ...
  #[local]
  Hint Resolve eqk_trans eqke_trans eqk_refl eqke_refl : smtcoq_farray.
  #[local]
  Hint Immediate eqk_sym eqke_sym : smtcoq_farray.

  Global Instance eqk_equiv : Equivalence eqk.
  Proof. split; eauto with smtcoq_farray. Qed.
  ...
  Qed.
  #[local]
  Hint Resolve InA_eqke_eqk : smtcoq_farray.
  ...
  #[local]
  Hint Unfold MapsTo In : smtcoq_farray.
  ...
  - exists y; auto with smtcoq_farray.
  - induction H as [a l eq|a l H IH].
    + destruct a as [k' y'].
      exists y'; auto with smtcoq_farray.
    + destruct IH as [e H0].
      exists e; auto with smtcoq_farray.
  Qed.

  Lemma MapsTo_eq : forall l x y e, eq x y -> MapsTo x e l -> MapsTo y e l.
  Proof.
    intros l x y e **; unfold MapsTo in *; apply InA_eqA with (x,e); auto with smtcoq_farray using eqke_equiv.
  Qed.

  Lemma In_eq : forall l x y, eq x y -> In x l -> In y l.
  Proof.
  destruct 2 as (e,E); exists e; eapply MapsTo_eq; eauto with smtcoq_farray.
  Qed.

  Lemma In_inv : forall k k' e l, In k ((k',e) :: l) -> eq k k' \/ In k l.
  Proof.
    inversion 1 as [? H0].
    inversion_clear H0 as [? ? H1|]; eauto with smtcoq_farray.
  ...
  (* This is to be removed once what is before compiles *)
...
#[local]
Hint Resolve empty_1 : smtcoq_farray.

Lemma empty_NoDup : NoDupA empty.
Proof.
 unfold empty; auto with smtcoq_farray.
Qed.
...
Proof.
 ...
 intros m.
 case m;auto with smtcoq_farray.
 intros p l inlist.
 destruct p.
 absurd (InA eqke (t0, e) ((t0, e) :: l));auto with smtcoq_farray.
Qed.
...
Proof.
 ...
 - destruct a; destruct X.eq_dec; [reflexivity|]; apply IHm.
   ...
   + inversion_clear belong1; inversion_clear H; [elim n; apply H0|exists x0; auto with smtcoq_farray].
Qed.
...
Proof.
 ...
 induction m; simpl;intros e' eqfind; inversion eqfind; auto with smtcoq_farray.
 destruct a, X.eq_dec.
...
Proof.
 ...
 - inversion_clear Hdup.
   ...
   + elim n; apply H1.
   + elim H; apply InA_eqk with (x,e'); auto with smtcoq_farray.
   + apply IHm; auto with smtcoq_farray.
Qed.
...
Proof.
 induction m; simpl; auto with smtcoq_farray; destruct a; intros.
 inversion_clear Hm.
 rewrite (IHm H1 x x'); auto with smtcoq_farray.
 destruct (X.eq_dec x t0) as [|Hneq]; destruct (X.eq_dec x' t0) as [|?Hneq'];
   trivial.
 - elim Hneq'; apply X.eq_trans with x; auto with smtcoq_farray.
 - elim Hneq; apply X.eq_trans with x'; auto with smtcoq_farray.
Qed.
...
Proof.
 induction m as [|[a m]]; intros x y e He; simpl in *; auto with smtcoq_farray.
 destruct X.eq_dec; [now auto with smtcoq_farray|].
...
Proof.
 ...
 - inversion_clear Hm.
 - inversion_clear Hm; destruct X.eq_dec.
   + elim H; apply X.eq_trans with a; [auto with smtcoq_farray|apply X.eq_sym; apply H0].
   + apply InA_cons_hd; apply H0.
   + apply InA_cons_tl; assumption.
   + apply InA_cons_tl; apply IHm; auto with smtcoq_farray.
Qed.
...
Proof.
 ...
 - simpl in Hm; destruct X.eq_dec.
   + apply InA_cons_tl; apply InA_cons in Hm; destruct Hm; [|now auto with smtcoq_farray].
     elim H; apply X.eq_sym; apply H0.
   + apply InA_cons in Hm; destruct Hm.
     * apply InA_cons_hd; auto with smtcoq_farray.
     * apply InA_cons_tl; eapply IHm; eauto with smtcoq_farray.
Qed.
...
Proof.
 induction m as [|[a m]]; intros x y e e' H Hm; simpl in *.
 - inversion_clear Hm; [|now auto with smtcoq_farray].
   compute in H0; elim H; auto with smtcoq_farray.
 - destruct X.eq_dec; simpl in *.
   + apply InA_cons in Hm; destruct Hm; [elim H; apply X.eq_sym; apply H0|].
     apply InA_cons_tl; auto with smtcoq_farray.
   + apply InA_cons in Hm; destruct Hm; [apply InA_cons_hd; auto with smtcoq_farray|].
     apply InA_cons_tl; eapply IHm; eauto with smtcoq_farray.
Qed.
...
Proof.
 induction m.
 - simpl; constructor; auto with smtcoq_farray; red; inversion 1.
 - intros.
   destruct a as (x',e').
   simpl; case (X.eq_dec x x'); inversion_clear Hm; auto with smtcoq_farray.
   + constructor; auto with smtcoq_farray.
     contradict H.
     apply InA_eqk with (x,e); auto with smtcoq_farray.
   + constructor; auto with smtcoq_farray.
     contradict H; apply add_3' with x e; auto with smtcoq_farray.
Qed.
...
Proof.
 intros.
 apply find_1; auto with smtcoq_farray.
 - apply add_NoDup; auto with smtcoq_farray.
 - apply add_1; auto with smtcoq_farray.
Qed.
...
Proof.
 ...
 - apply find_1; auto with smtcoq_farray.
   + apply add_NoDup; auto with smtcoq_farray.
   + apply add_2; auto with smtcoq_farray.
     apply find_2; auto with smtcoq_farray.
 - case_eq (find x (add a e m)); intros; auto with smtcoq_farray.
   rewrite <- H0; symmetry.
   apply find_1; auto with smtcoq_farray.
   apply add_3 with a e; auto with smtcoq_farray.
   apply find_2; auto with smtcoq_farray.
Qed.
...
Proof.
 ...
 - inversion_clear Hm.
   ...
   + intros [e' ?]; elim H0.
     ...
     * apply X.eq_trans with x; [|auto with smtcoq_farray].
       apply X.eq_sym; auto with smtcoq_farray.
     * apply InA_eqke_eqk; auto with smtcoq_farray.
   + intros [e' H2]; apply InA_cons in H2; destruct H2.
     * elim n; apply X.eq_trans with y; [auto with smtcoq_farray|apply H2].
     * elim IHm with x y; auto with smtcoq_farray.
       exists e'; auto with smtcoq_farray.
Qed.
...
Proof.
 ...
 + apply InA_cons in He; destruct He, X.eq_dec.
   ...
   - inversion_clear Hm.
     ...
     * elim H1; apply InA_eqk with (y, e); [assumption|]; apply InA_eqke_eqk; auto with smtcoq_farray.
     * right; apply IHm; auto with smtcoq_farray.
Qed.

Lemma remove_3 : forall m (Hm:NoDupA m) x y e,
  MapsTo y e (remove x m) -> MapsTo y e m.
Proof.
 induction m as [|[a m]]; intros Hm x y e H; unfold PX.MapsTo; simpl in *; auto with smtcoq_farray.
 destruct X.eq_dec.
 - apply InA_cons_tl; apply H.
 - inversion_clear Hm; apply InA_cons in H; destruct H; [apply InA_cons_hd; auto with smtcoq_farray|].
   apply InA_cons_tl; apply IHm with x; auto with smtcoq_farray.
Qed.
...
Proof.
 ...
 - destruct X.eq_dec.
   + apply InA_cons_tl; auto with smtcoq_farray.
   + apply InA_cons in H; destruct H; [apply InA_cons_hd; auto with smtcoq_farray|].
     inversion_clear Hm; apply InA_cons_tl; apply IHm with x; auto with smtcoq_farray.
Qed.
...
Proof.
 ...
 - intros.
   inversion_clear Hm.
   destruct a as (x',e').
   simpl; case (X.eq_dec x x'); auto with smtcoq_farray.
   constructor; auto with smtcoq_farray.
   contradict H; apply remove_3' with x; auto with smtcoq_farray.
Qed.
...
Proof.
 induction m as [|[a m]]; intros A i f; simpl; auto with smtcoq_farray.
Qed.
...
Proof.
 ...
 induction m.
 - simpl; auto with smtcoq_farray.
 - destruct a; simpl; intros.
   destruct H.
   inversion_clear Hm.
   assert (H3 : In t0 m').
   + apply H; exists e; auto with smtcoq_farray.
   + destruct H3 as (e', H3).
     unfold check at 2; rewrite (find_1 Hm' H3).
     rewrite (H0 t0); simpl; auto with smtcoq_farray.
     eapply IHm; auto with smtcoq_farray.
     split; intuition.
     * apply H.
       destruct H5 as (e'',H5); exists e''; auto with smtcoq_farray.
     * apply H0 with k; auto with smtcoq_farray.
Qed.
...
Proof.
 ...
 - destruct a; simpl; intros.
   ...
   + clear H1 H0 Hm' IHm.
     ...
     induction m; simpl; auto with smtcoq_farray; intros.
     destruct a; simpl in *.
     destruct (andb_prop _ _ (IHm _ H)); auto with smtcoq_farray.
   + rewrite H2 in H.
     destruct (IHm H1 m' Hm' cmp H); auto with smtcoq_farray.
     unfold check in H2.
     ...
     * destruct H6 as (e0,H6); inversion_clear H6.
       -- compute in H7; destruct H7; subst.
          exists e'.
          apply PX.MapsTo_eq with t0; auto with smtcoq_farray.
          apply find_2; auto with smtcoq_farray.
       -- apply H3.
          exists e0; auto with smtcoq_farray.
     * inversion_clear H6.
       -- compute in H8; destruct H8; subst.
          rewrite (find_1 Hm' (PX.MapsTo_eq H6 H7)) in H5; congruence.
       -- apply H4 with k; auto with smtcoq_farray.
Qed.
...
Proof.
 ...
 - destruct a as (x',e').
   ...
   + constructor 2.
     unfold MapsTo in *; auto with smtcoq_farray.
Qed.
...
Proof.
 ...
 - destruct a as (x',e).
   ...
   + destruct IHm as (e'',hyp).
     * exists e'; auto with smtcoq_farray.
     * exists e''.
       constructor 2; auto with smtcoq_farray.
Qed.

Lemma map_NoDup : forall m (Hm : NoDupA (@eqk elt) m)(f:elt->elt'),
  NoDupA (@eqk elt') (map f m).
Proof.
 induction m; simpl; auto with smtcoq_farray.
 intros.
 destruct a as (x',e').
 inversion_clear Hm.
 constructor; auto with smtcoq_farray.
 ...
 clear IHm H0.
 induction m; simpl in *; auto with smtcoq_farray.
 - inversion H.
 - destruct a; inversion H; auto with smtcoq_farray.
Qed.
...
Proof.
 ...
 - destruct a as (x',e').
   ...
   + exists x'.
     ...
     split; auto with smtcoq_farray.
     constructor 1.
     unfold eqke in *; simpl in *; intuition congruence.
   + destruct IHm as (y, hyp); auto with smtcoq_farray.
     exists y; intuition.
Qed.
...
Proof.
 ...
 intros.
 destruct a as (x',e').
 inversion_clear Hm; auto with smtcoq_farray.
 constructor; auto with smtcoq_farray.
 contradict H.
 clear IHm H0.
 induction m; simpl in *; auto with smtcoq_farray.
 - inversion_clear H.
 - destruct a; inversion_clear H; auto with smtcoq_farray.
Qed.
...
Proof.
 induction l; simpl; auto with smtcoq_farray.
 destruct a; simpl; auto with smtcoq_farray.
 inversion_clear 1.
 intros; apply add_NoDup; auto with smtcoq_farray.
Qed.
#[local]
...
Proof.
 ...
 - simpl.
   apply find_1.
   + apply mapi_NoDup; auto with smtcoq_farray.
   + destruct (mapi_1 (fun k e => (Some e, find k m')) (find_2 H)) as (y,(H0,H1)).
     rewrite (find_eq Hm' (X.eq_sym H0)); auto with smtcoq_farray.
 - simpl.
   case_eq (find x (mapi (fun k e => (Some e, find k m')) m)); intros; auto with smtcoq_farray.
   destruct (@mapi_2 _ _ m x (fun k e => (Some e, find k m'))).
   + exists p; apply find_2; auto with smtcoq_farray.
...
Proof.
 ...
 - simpl.
   ...
   + destruct (mapi_1 (fun k e => (find k m, Some e)) (find_2 H)) as (y,(H0,H1)).
     rewrite (find_eq Hm (X.eq_sym H0)); auto with smtcoq_farray.
 - simpl.
   case_eq (find x (mapi (fun k e' => (find k m, Some e')) m')); intros; auto with smtcoq_farray.
...
Proof.
 ...
 assert (NoDupA (eqk (elt:=oee')) (combine_l m m')). {
   unfold combine_l; apply mapi_NoDup; auto with smtcoq_farray.
 }
 assert (NoDupA (eqk (elt:=oee')) (combine_r m m')). {
   unfold combine_r; apply mapi_NoDup; auto with smtcoq_farray.
 }
 ...
 clear Hm' Hm m m'.
 induction l.
 - destruct o; destruct o'; simpl; intros; discriminate || auto with smtcoq_farray.
 - destruct a; simpl in *; intros.
   destruct (X.eq_dec x t0); simpl in *.
   + unfold at_least_left in H1.
     destruct o; simpl in *; try discriminate.
     inversion H1; subst.
     apply add_eq; auto with smtcoq_farray.
     inversion_clear H; auto with smtcoq_farray.
   + inversion_clear H.
     rewrite <- IHl; auto with smtcoq_farray.
     apply add_not_eq; auto with smtcoq_farray.
Qed.
...
Proof.
 ...
 - simpl; auto with smtcoq_farray.
 - inversion_clear H1.
   destruct a; destruct o; simpl; auto with smtcoq_farray.
   constructor; auto with smtcoq_farray.
   contradict H.
   clear IHl1.
   induction l1.
   + inversion H.
   + inversion_clear H0.
     destruct a; destruct o; simpl in *; auto with smtcoq_farray.
     inversion_clear H; auto with smtcoq_farray.
Qed.
...
Proof.
 ...
 - destruct o; destruct o'; simpl in *; try discriminate; auto with smtcoq_farray.
 - destruct a as (k,(oo,oo')); simpl in *.
   inversion_clear H2.
   destruct (X.eq_dec x k) as [|Hneq]; simpl in *.
   + (* x = k *)
     assert (at_least_one_then_f o o' = f oo oo').
     * destruct o; destruct o'; simpl in *; inversion_clear H; auto with smtcoq_farray.
     * rewrite H2.
       unfold f'; simpl.
       destruct (f oo oo'); simpl.
       -- destruct (X.eq_dec x k) as [|Hneq]; try contradict Hneq; auto with smtcoq_farray.
       -- destruct (IHm0 H1) as (_,H4); apply H4; auto with smtcoq_farray.
          case_eq (find x m0); intros; auto with smtcoq_farray.
          elim H0.
          apply InA_eqk with (x,p); auto with smtcoq_farray.
          apply InA_eqke_eqk.
          exact (find_2 H3).
   + (* k < x *)
     unfold f'; simpl.
     destruct (f oo oo'); simpl.
     * destruct (X.eq_dec x k); [ contradict Hneq; auto with smtcoq_farray | auto with smtcoq_farray].
       destruct (IHm0 H1) as (H3,_); apply H3; auto with smtcoq_farray.
     * destruct (IHm0 H1) as (H3,_); apply H3; auto with smtcoq_farray.
...
Proof.
 intros.
 rewrite map2_0; auto with smtcoq_farray.
 destruct H as [(e,H)|(e,H)].
 - rewrite (find_1 Hm H).
   destruct (find x m'); simpl; auto with smtcoq_farray.
 - rewrite (find_1 Hm' H).
   destruct (find x m); simpl; auto with smtcoq_farray.
Qed.
...
Proof.
 ...
 - left; exists e0; auto with smtcoq_farray.
 - left; exists e0; auto with smtcoq_farray.
 - right; exists e0; auto with smtcoq_farray.
 - discriminate.
Qed.
