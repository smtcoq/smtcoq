...
Section FArray.
 ...
 (* Qed. *)
  ...
  (*   Qed. *)
    ...
    (* Lemma diff_index_p : forall a b, equal a b = false -> {i | select a i <> select b i}. *)
    (* Proof. *)
    (*   intros a b. *)
    ...
    (*   (* } *) *)
    (*   assert (HInd: forall x y (xS:NoDupA _ x) (yS:NoDupA _ y) (xD:NoDefault x) (yD:NoDefault y), *)
    (*              let a := Build_farray xS xD in *)
    (*              let b := Build_farray yS yD in *)
    (*              equal a b = false -> {i : key | select a i <> select b i}). *)
    (*   { *)
    ...
    (*   } *)
    (*   intro H. *)
    (*   apply (HInd (this a) (this b) (NoDup a) (NoDup b) (nodefault a) (nodefault b)). *)
