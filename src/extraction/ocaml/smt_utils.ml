...
let declare_assert = Smtlib2_genConstr.make_root
...
let to_coq = to_coq (fun i -> mkInt (SmtAtom.Form.to_lit i)) certif_ops


(* From verit/verit.ml and trace/smtCommands.ml *)
let checker ra rf roots max_id confl =
  let (tres,last_root) = to_coq confl in
  let certif =
