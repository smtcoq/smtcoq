...
module type S = sig
  ...
  val to_clause :
    term ->
    VeritSyntax.verit_state ->
    State.smtlib_state -> clause
  ...
  val mk_clause_cl : ?reuse:bool -> rule -> term list -> int list -> VeritSyntax.verit_state -> State.smtlib_state -> int

  (** Create an input unit clause. It is given an identifier that is not
      returned. *)
  val mk_input : Hstring.t -> term -> VeritSyntax.verit_state -> State.smtlib_state -> unit

  val mk_admit_preproc : Hstring.t -> term -> VeritSyntax.verit_state -> State.smtlib_state -> unit
  ...
  val register_decl : Hstring.t -> term -> VeritSyntax.verit_state -> State.smtlib_state -> unit
