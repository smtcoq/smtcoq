...
let lfsc_parse_last lb =
  RocqInterface.raise_debug "Type-checking LFSC proof...@?";
  let t0 = Sys.time () in
...
let checker fsmt fproof =
  ...
  RocqInterface.raise_debug "Coq checker...@.";
  let t0 = Sys.time () in
  let r = SmtCommands.checker c in
  let t1 = Sys.time () in
  RocqInterface.raise_debug "Done (Coq) [%.3f s]@." (t1 -. t0);
  r
  ...
  let proof =
    ...
    | Unsat ->
      begin
        ...
        with
        | Ast.CVC4Sat -> RocqInterface.raise_error "CVC4 returned SAT"
        | No_proof -> RocqInterface.raise_error "CVC4 did not generate a proof"
        | Failure s -> RocqInterface.raise_error "Importing of proof failed: %s" s
      end
    | Sat ->
...
let get_model_from_file filename =
  ...
  | [SExpr.Atom "sat"; m] -> m
  | _ -> RocqInterface.raise_error "CVC4 returned SAT but no model"
...
let call_cvc4_file _ env rt ro ra rf root =
  ...
  RocqInterface.raise_debug "%s" cvc4_cmd;
  let t0 = Sys.time () in
  ...
  let t1 = Sys.time () in
  RocqInterface.raise_debug "CVC4 = %.5f" (t1 -. t0);

  if exit_code <> 0 then begin
    RocqInterface.raise_error "CVC4 crashed: return code %d" exit_code
  end;
