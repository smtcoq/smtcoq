...
From SMTCoq.utils Require Import Classes.
...
(* Goal forall (A B C:Type) (HA:Classes.CompDec A) (a1 a2:A) (b1 b2 b3 b4:B) (c1 c2:C), *)
...
Ltac add_compdecs_term u :=
  ...
  | context C [@Logic.eq ?t _ _] => 
    ...
    | false =>
      ...
      | _ : Classes.CompDec t |- _ => add_compdecs_term u'
      (* Otherwise, add it in the local context *)
      | _ =>
        let p := fresh "p" in
        assert (p: Classes.CompDec t);
...
Ltac collect_compdecs :=
  match goal with
  | H : Classes.CompDec ?T |- _ =>
    let _ := match goal with _ => change (id (Classes.CompDec T)) in H end in
    let acc := collect_compdecs in
    let _ := match goal with _ => change (Classes.CompDec T) in H end in
    let res :=
        lazymatch is_interpreted_type T with
        | true => constr:(acc)
        | false => constr:((acc, H))
...
Ltac generate_rels compdecs :=
  ...
  | (?compdecs', ?HA) =>
    ...
    lazymatch ty with
    | Classes.CompDec ?A =>
      let rel := constr:((2%nat, @eq A, @Classes.eqb_of_compdec A HA, @Classes.compdec_eq_eqb A HA)) in
...
(* Section Test. *)
(*   Variables (A:Type) (HA:Classes.CompDec A). *)

(*   Goal forall (a1 a2:A), a1 = a2. *)
(*   Proof. *)
(*     intros a1 a2. *)
(*     trakt Z bool with rel (2%nat, @eq A, @Classes.eqb_of_compdec A HA, @Classes.compdec_eq_eqb A HA). *)
(*   Abort. *)
(* End Test. *)
...
Ltac remove_compdec_hyp H :=
  let TH := type of H in
  match TH with
  | forall p : Classes.CompDec ?A, _ =>
    match goal with
    | [ p' : Classes.CompDec A |- _ ] =>
      let H1 := fresh in
      assert (H1 := H p'); clear H; assert (H := H1); clear H1;
      remove_compdec_hyp H
    | _ =>
      let c := fresh "c" in
      assert (c : Classes.CompDec A);
      [ try (exact _)
      | let H1 := fresh in
